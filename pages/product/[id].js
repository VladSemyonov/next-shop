...
export default function Product(props) {
  ...
  useEffect(
    () =>
      fetch(
        `http://vladreact.me/server/product/${id}`,
        {
          method: "GET",
        }
      )
  ...
  useEffect(()=>setMiddleScore(Math.floor(product?.param?.length / 2) - 1), [product])
        ...
        <div className="row  mb--60" style={{ maxWidth: "100%" }}>
          <div className="col-lg-5 mb--30">
            ...
            <div className="add-cart-btn mt--40 d-flex justify-content-center">
              <button className="btn font-bold" onClick={()=>addToBascket(product)}>
                <span className="text-center align-middle">Добавить в корзину</span>
              </button>
            </div>
          </div>
