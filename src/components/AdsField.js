...
export default function AdsField({ title, query }) {
    ...
    return (
        <div className={"container"} style={{ marginBottom: "60px" }}>
            ...
            <div style={{ borderLeft: "1px solid lightgrey", borderTop: "1px solid lightgrey", boxShadow: "inset 0 0 5px gray", padding: "0 4px"  }}>
                {items && <Carousel itemWidth={350} >
                    {items.map((item, index) => <div key={index} className={`col-12 mys`}>
                        <div className="product-card">
                            <div className="product-grid-content">
                                <div className="product-header">
                                    <span style={{ height: "80px" }}>
                                        <a
                                            to={`/product/${item?._attributes.id}`}>{item.name._text}
                                        </a>
                                    </span>
                                </div>
                                <div className="product-card--body">
                                    ...
                                    <div className="price-block">
                                        <div className='d-flex align-items-center justify-content-center'>
                                            <span
                                                className="price">{item.price._text} {item.currencyId._text}
                                            </span>
                                        </div>
                                    </div>
                                </div>
